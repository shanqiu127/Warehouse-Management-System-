...
一个面向企业多部门协同与运营治理的管理系统，以进销退存与库存预警为核心业务主线，采用前后端分离架构，覆盖认证授权、用户与部门管理、基础资料、统计分析等核心能力。

在进销存流程基础上，系统进一步补齐了工作要求闭环、历史单据审批作废/红冲、公告定向投放、站内消息提醒、超时治理，以及面向项目知识问答的 AI 助手能力，使系统同时具备业务处理、部门协同、流程追踪与智能辅助支持等实际落地价值。

## 🎯 适用业务场景 / 适合谁用

本项目适合采购、销售、仓配、财务、人事等多角色协同的中小型企业，也适合希望把进销存管理、部门协作、审批流转与项目内 AI 辅助问答放在同一套系统中的团队。
...
| 审批追溯 | 历史单据不能直接处理，需经审批中心（仓储管理员）审批后作废或红冲 | 保留审批轨迹与处理结果，提升业务可控性 |
